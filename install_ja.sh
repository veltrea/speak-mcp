...
if [ -d "$HOME/.claude" ]; then
    CONFIGURED_ANY=true
    echo "--------------------------------------"
    echo "[Claude Code] が検出されました。"
    echo "  MCP設定  : $CLAUDE_CODE_MCP"
    echo "  Settings : $CLAUDE_CODE_SETTINGS"
    echo "  設定後、Claude Code を再起動してください。"
    printf "  speak-mcp を自動設定しますか? [y/N]: "
    read -r answer
    case "$answer" in
        [yY]*)
            # 1. ~/.claude/.mcp.json に追加
            ...
            if grep -q '"speak"' "$CLAUDE_CODE_MCP" 2>/dev/null; then
                echo "[SKIP] Claude Code MCP: speak は既に設定済みです。"
            else
...
with open(path, 'w') as f:
    ...
    f.write('\n')
PYEOF
                echo "[OK] Claude Code MCP: 設定を追記しました。"
            fi

            # 2. settings.json に enabledMcpjsonServers を追加
            if [ -f "$CLAUDE_CODE_SETTINGS" ]; then
                if grep -q '"enabledMcpjsonServers"' "$CLAUDE_CODE_SETTINGS" 2>/dev/null; then
                    if grep -q '"speak"' "$CLAUDE_CODE_SETTINGS" 2>/dev/null; then
                        echo "[SKIP] Claude Code Settings: speak は既に enabledMcpjsonServers に含まれています。"
                    else
...
with open(path, 'w') as f:
    ...
    f.write('\n')
PYEOF
                        echo "[OK] Claude Code Settings: enabledMcpjsonServers に speak を追加しました。"
                    fi
                else
                    python3 - "$CLAUDE_CODE_SETTINGS" <<'PYEOF'
import json, sys
...
with open(path, 'w') as f:
    ...
    f.write('\n')
PYEOF
                    echo "[OK] Claude Code Settings: enabledMcpjsonServers を追加しました。"
                fi
            else
                echo "[INFO] settings.json が見つかりません。新規作成します..."
                cat > "$CLAUDE_CODE_SETTINGS" <<'SETTINGSEOF'
{
  "enabledMcpjsonServers": ["speak"]
}
SETTINGSEOF
                echo "[OK] Claude Code Settings: enabledMcpjsonServers を含む settings.json を作成しました。"
            fi
            ;;
        *)
            echo "[SKIP] スキップしました。"
