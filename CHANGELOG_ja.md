...
- `install.sh` / `install_ja.sh`: Claude Code の設定先を修正 — `settings.json` に `mcpServers` を追加していたがスキーマバリデーションで拒否される問題を修正。`~/.claude/.mcp.json` への MCP 登録と `settings.json` への `enabledMcpjsonServers` 追加に変更
- `install.sh` / `install_ja.sh`: `settings.json` が存在しない場合、警告表示のみだったのを `enabledMcpjsonServers` 付きで新規作成するように修正
