...
if [ -d "$HOME/.claude" ]; then
    ...
    echo "  MCP config : $CLAUDE_CODE_MCP"
    echo "  Settings   : $CLAUDE_CODE_SETTINGS"
    echo "  Please restart Claude Code after configuration."
    printf "  Auto-configure speak-mcp? [y/N]: "
    read -r answer
    case "$answer" in
        [yY]*)
            ...
            if [ ! -f "$CLAUDE_CODE_MCP" ]; then
                ...
                echo '{"mcpServers": {}}' > "$CLAUDE_CODE_MCP"
            fi
            if grep -q '"speak"' "$CLAUDE_CODE_MCP" 2>/dev/null; then
                echo "[SKIP] Claude Code MCP: 'speak' is already configured."
            else
                python3 - "$CLAUDE_CODE_MCP" "$BINARY_PATH" <<'PYEOF'
import json, sys
path, binary = sys.argv[1], sys.argv[2]
with open(path, 'r') as f:
    data = json.load(f)
...
with open(path, 'w') as f:
    ...
    f.write('\n')
PYEOF
                echo "[OK] Claude Code MCP: Configuration updated."
            fi

            # 2. Add enabledMcpjsonServers to settings.json
            if [ -f "$CLAUDE_CODE_SETTINGS" ]; then
                if grep -q '"enabledMcpjsonServers"' "$CLAUDE_CODE_SETTINGS" 2>/dev/null; then
                    if grep -q '"speak"' "$CLAUDE_CODE_SETTINGS" 2>/dev/null; then
                        echo "[SKIP] Claude Code Settings: 'speak' already in enabledMcpjsonServers."
                    else
                        python3 - "$CLAUDE_CODE_SETTINGS" <<'PYEOF'
import json, sys
path = sys.argv[1]
with open(path, 'r') as f:
    data = json.load(f)
servers = data.get('enabledMcpjsonServers', [])
if 'speak' not in servers:
    servers.append('speak')
    data['enabledMcpjsonServers'] = servers
with open(path, 'w') as f:
    json.dump(data, f, indent=2, ensure_ascii=False)
    f.write('\n')
PYEOF
                        echo "[OK] Claude Code Settings: Added 'speak' to enabledMcpjsonServers."
                    fi
                else
                    python3 - "$CLAUDE_CODE_SETTINGS" <<'PYEOF'
import json, sys
...
with open(path, 'r') as f:
    data = json.load(f)
data['enabledMcpjsonServers'] = ['speak']
with open(path, 'w') as f:
    json.dump(data, f, indent=2, ensure_ascii=False)
    f.write('\n')
PYEOF
                    echo "[OK] Claude Code Settings: Added enabledMcpjsonServers."
                fi
            else
                echo "[INFO] settings.json not found. Creating with enabledMcpjsonServers..."
                cat > "$CLAUDE_CODE_SETTINGS" <<'SETTINGSEOF'
{
  "enabledMcpjsonServers": ["speak"]
}
SETTINGSEOF
                echo "[OK] Claude Code Settings: Created with enabledMcpjsonServers."
            fi
            ;;
        *)
            echo "[SKIP] Skipped."
